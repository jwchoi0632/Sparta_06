...
void ARandomSpawner::InitializeMovePlatform(int32 index, FVector startPos, FVector scale)
{
	...
	if (type != EMoveType::RotateOnly) //only move || mvoe and rotate
	{
		...
		int32 Distance = FMath::RandRange(300, 1000);
		platform->SetMoveDirection(DirectionType);
		platform->SetMoveDistance(Distance);
		platform->SetMoveSpeed(FMath::RandRange(200, 1000));
		platform->SetIsReturn(true); // 임시

		switch (DirectionType)
		{
		case EDirectionType::X: NextLocation += FVector(Distance, 0, 0); break;
		case EDirectionType::Y: NextLocation += FVector(0, Distance, 0); break;
		case EDirectionType::Z: NextLocation += FVector(0, 0, Distance); break;
		case EDirectionType::XY: NextLocation += FVector(Distance, Distance, 0); break;
		case EDirectionType::XZ: NextLocation += FVector(Distance, 0, Distance); break;
		case EDirectionType::YZ: NextLocation += FVector(0, Distance, Distance); break;
	...
	if (type != EMoveType::MoveOnly) //only rotate || move and rotate
	{
		...
		platform->SetRotateSpeed(FMath::RandRange(60, 360));
		NextLocation += FVector::One() * 100;
	}
}
...
void ARandomSpawner::InitializeDestroyingPlatform(int32 index, FVector startPos, FVector scale)
{
	...
	platform->SetDestroyTime(FMath::RandRange(0.2f, 0.5f));
